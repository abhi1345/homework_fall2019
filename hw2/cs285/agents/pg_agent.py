...
class PGAgent(BaseAgent):
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)):
            ...
            discounted_rtg = self.gamma**(indices - start_time_index) * np.array(rewards[start_time_index:])

            # 4) calculate a scalar: sum_{t'=t}^{T-1} gamma^(t'-t) * r_{t'}
            sum_discounted_rtg = np.sum(self.gamma**(indices - start_time_index) * rewards[indices])
