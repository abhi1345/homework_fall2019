...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        """
        counts = np.ones(len(states))
        for i, state in enumerate(states):
            bin_name = self.preprocessor(state)
            value = self.hist[bin_name]
            counts[i] = value
        return counts
    ...
    def get_prob(self, states):
        ...
        for i, state in enumerate(states):
            bin_name = self.preprocessor(state)
            value = self.hist[bin_name]
            probs[i] = value / float(self.total)
        return probs
...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            euc_dists = np.sum(np.power(deltas, 2), axis=2)
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        self.update_op = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(-1 * self.elbo)
    ...
    def update(self, state1, state2, target):
        ...
        op, elbo, ll, kl = self.sess.run([self.update_op, self.elbo, self.log_likelihood, self.kl], \
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run([self.likelihood], feed_dict = \
            { self.state1 : state1, \
              self.state2 : state2, \
              self.discrim_target : np.ones([state2.shape[0], 1])})
        return likelihood
